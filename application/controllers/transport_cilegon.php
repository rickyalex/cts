...
class Transport_cilegon extends CI_Controller
{
    function __construct(){
		parent::__construct();
		$this->load->helper('url','language');
		$this->load->library(array('template','ion_auth','form_validation'));
		$group = array('admin', 'transport cilegon');
		if(!$this->ion_auth->in_group($group)){
			$this->session->set_flashdata('message', 'You must be a Transport Cilegon Manager to view this page');
			redirect('dashboard');
	  	}
	}
    public function index()
    {
        ...
        $xcrud->pass_var('project_name','TRANSPORT CILEGON', 'create');
        ...
        $this->template->display('transport/cilegon',$data);
    }
}
