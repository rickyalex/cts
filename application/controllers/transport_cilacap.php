...
class Transport_cilacap extends CI_Controller
{
    function __construct(){
		parent::__construct();
		$this->load->helper('url','language');
		$this->load->library(array('template','ion_auth','form_validation'));
		$group = array('admin', 'transport cilacap');
		if(!$this->ion_auth->in_group($group)){
			$this->session->set_flashdata('message', 'You must be a Transport Cilacap Manager to view this page');
			redirect('dashboard');
	  	}
	}
    public function index()
    {
        ...
        $xcrud->pass_var('project_name','TRANSPORT CILACAP', 'create');
        ...
        $this->template->display('transport/cilacap',$data);
    }
}
