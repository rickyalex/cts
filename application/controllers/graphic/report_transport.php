...
class Report_transport extends CI_Controller{
    
    function __construct(){
		parent::__construct();
		$this->load->model('m_report_transport','',TRUE);
    ...
    function index(){
        foreach($this->m_report_transport->revenue()->result_array() as $row){
		...
		foreach($this->m_report_transport->target()->result_array() as $row){
		...
		foreach($this->m_report_transport->ebitda()->result_array() as $row){
		...
		foreach($this->m_report_transport->target_ebitda()->result_array() as $row){
		...
		foreach($this->m_report_transport->cost()->result_array() as $row){
		...
		foreach($this->m_report_transport->target_cost()->result_array() as $row){
		...
		foreach($this->m_report_transport->cost_revenue()->result_array() as $row){
		...
		foreach($this->m_report_transport->target_cost_revenue()->result_array() as $row){
        ...
        $this->template->display('graphic/report_transport',$data);
	}
}
