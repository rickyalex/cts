...
class Report_warehouse extends CI_Controller{
    ...
    function __construct(){
		parent::__construct();
		$this->load->model('m_report_warehouse','',TRUE);
    ...
    function index(){
        foreach($this->m_report_warehouse->revenue()->result_array() as $row){
		...
		foreach($this->m_report_warehouse->target()->result_array() as $row){
		...
		foreach($this->m_report_warehouse->ebitda()->result_array() as $row){
		...
		foreach($this->m_report_warehouse->target_ebitda()->result_array() as $row){
		...
		foreach($this->m_report_warehouse->cost()->result_array() as $row){
		...
		foreach($this->m_report_warehouse->target_cost()->result_array() as $row){
		...
		foreach($this->m_report_warehouse->cost_revenue()->result_array() as $row){
		...
		foreach($this->m_report_warehouse->target_cost_revenue()->result_array() as $row){
        ...
        $this->template->display('graphic/report_warehouse',$data);
	}
}
