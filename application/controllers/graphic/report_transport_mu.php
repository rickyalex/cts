...
class Report_transport_mu extends CI_Controller{
    
    function __construct(){
		parent::__construct();
		$this->load->model('m_report_transport_mu','',TRUE);
		$this->load->helper('url','language');
		$this->load->library(array('template','ion_auth','form_validation'));
		$group = array('admin', 'bd','direksi','packaging','warehouse1','warehouse2','warehouse3','warehouseclg','warehousebsicbt','transport cilegon','transport dump truck','transport narogong','transport warehouse','transport mu','freight forwarding','hrd');
    ...
    function index(){
        foreach($this->m_report_transport_mu->revenue()->result_array() as $row){
		...
		foreach($this->m_report_transport_mu->target()->result_array() as $row){
		...
		foreach($this->m_report_transport_mu->ebitda()->result_array() as $row){
		...
		foreach($this->m_report_transport_mu->target_ebitda()->result_array() as $row){
		...
		foreach($this->m_report_transport_mu->cost()->result_array() as $row){
		...
		foreach($this->m_report_transport_mu->target_cost()->result_array() as $row){
		...
		foreach($this->m_report_transport_mu->cost_revenue()->result_array() as $row){
		...
		foreach($this->m_report_transport_mu->target_cost_revenue()->result_array() as $row){
        ...
        $this->template->display('graphic/report_transport_mu',$data);
	}
}
