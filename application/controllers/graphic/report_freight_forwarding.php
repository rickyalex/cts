...
class Report_freight_forwarding extends CI_Controller{
    
    function __construct(){
		parent::__construct();
		$this->load->model('m_report_freight_forwarding','',TRUE);
    ...
    function index(){
        foreach($this->m_report_freight_forwarding->revenue()->result_array() as $row){
		...
		foreach($this->m_report_freight_forwarding->target()->result_array() as $row){
		...
		foreach($this->m_report_freight_forwarding->ebitda()->result_array() as $row){
		...
		foreach($this->m_report_freight_forwarding->target_ebitda()->result_array() as $row){
		...
		foreach($this->m_report_freight_forwarding->cost()->result_array() as $row){
		...
		foreach($this->m_report_freight_forwarding->target_cost()->result_array() as $row){
		...
		foreach($this->m_report_freight_forwarding->cost_revenue()->result_array() as $row){
		...
		foreach($this->m_report_freight_forwarding->target_cost_revenue()->result_array() as $row){
        ...
        $this->template->display('graphic/report_freight_forwarding',$data);
	}
}
