...
class Ion_auth_model extends CI_Model
{
 ...
 /**
	 ...
	 **/
	public function is_authorized($user_id, $link)
	{
		if (empty($user_id))
		{
			return FALSE;
		...
		if ($this->DB->join('menu','users_menu.menu = menu.id')->where(array('users_menu.user_id'=>$user_id,'menu.link_menu'=>$link))->get('users_menu')->num_rows() > 0){
			return true;
		}
		else return false;
	...
	/**
	 * Get the crud auth for this user
	 *
	 * @return array
	 * @author Ricky Alexander
	 **/
	public function get_crud_auth($user_id, $link_menu)
	{
		if (empty($user_id))
		{
			return FALSE;
		...
		if ($this->DB->join('menu','users_menu.menu = menu.id')->where(array('users_menu.user_id'=>$user_id, 'menu.link_menu'=>$link_menu))->get('users_menu')->num_rows() > 0){
			return $this->DB->join('menu','users_menu.menu = menu.id')->where(array('users_menu.user_id'=>$user_id, 'menu.link_menu'=>$link_menu))->get('users_menu')->row_array();
		}
		else return array();
